<template>
  ...
  <Drawer :placement="placement">
    <template #trigger="{ show, close }" :placement="placement">
      <Btn type="primary" @click="toShow('right', show)">右边弹出</Btn>
      <Btn type="primary" @click="toShow('left', show)">左边弹出</Btn>
      <Btn @click="toShow('center', show)">中间弹出</Btn>
    </template>
    <template #default>
      <div
        style="width: 300px"
      ...
      <div style="height: 200px; width: 400px" v-if="placement == 'center'">
        弹出
      </div>
    </template>
  </Drawer>
...
function beforeShow(p: string) {
  placement.value = p
  showDrawer.value = true
}
function toShow(p: string, show: () => void) {
  placement.value = p
  show()
}
</script>
