<template>
  ...
  <p>
    ...
    <Btn @click="add(1)" type="default" mode="line">change {{ count }}</Btn>
  </p>
  <p>
    监控下边盒子的滚动情况 scrollTop:{{ scrollTop }}；scrollBottom:{{
      scrollBottom
    }}
  </p>
  <div style="height: 400px; overflow: auto" ref="scrollElement">
    <p style="height: 290px; background-color: #f1f1f1"></p>
    <Layer trigger="click">
      <template #trigger>
        <Btn v-if="count % 2 === 0" @click="toPrint('ou')">
          偶数：{{ count }}
        </Btn>
        <Btn v-else @click="toPrint('及')">奇数：{{ count }}</Btn>
      </template>
      <div>这是【弹出层】里要展示的一些东西</div>
    </Layer>
  ...
  <table :class="css.tb">
    <tbody>
      ...
      <tr>
        <td>
          <Layer placement="left-start" :arrow-offset="{ x: 0, y: 5 }">
            <template #trigger>
              <Btn>左边顶部对齐</Btn>
            </template>
            <div :class="css.body" style="max-width: 150px">
              <h3>三生石</h3>
      ...
      <tr>
        <td>
          <Layer placement="left">
            ...
            <div :class="css.body" style="max-width: 200px">
              <p>
        ...
        <td>
          <Layer placement="center">
            ...
            <div :class="css.body">
              <p>这是居中显示的弹出层，看看效果怎么样吧</p>
              <p>居中显示的弹出层，是没有箭头的</p>
            </div>
          </Layer>
        </td>
        <td>
          <Layer placement="client-center" hasMask>
...
import useScroll from 'will-ui/use/useScroll'
